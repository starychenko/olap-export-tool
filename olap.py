...
def save_credentials(username, password, encrypted=False):
    ...
    try:
        ...
        if encrypted:
            ...
            print_info(
                f"Облікові дані збережено та зашифровано з унікальним ідентифікатором пристрою"
            )
        else:
    ...
    try:
        ...
        if encrypted:
            ...
            with open(credentials_file, "r") as f:
                ...
                auth_username = (
                    username  # Зберігаємо ім'я користувача для подальшого використання
                )
                return username, password
    except Exception as e:
...
class TimeTracker:
    ...
    def __init__(self, total_items):
        ...
        self.last_item_end_time = (
            self.start_time
        )  # Час завершення обробки останнього елемента
        self.currently_waiting = (
            False  # Флаг, що показує, чи в режимі очікування ми зараз
        )
    ...
    def get_remaining_wait_time(self):
        ...
        wait_time_per_item = int(
            os.getenv("QUERY_TIMEOUT", 30)
        )  # Значення з .env або за замовчуванням 30 сек
    ...
    def get_progress_info(self):
        ...
        debug_output = os.getenv("DEBUG", "false").lower() in (
            "true",
            "1",
            "yes",
        )  # Контроль через змінну середовища
        if debug_output and self.elapsed_times and self.processed_items > 0:
            ...
            print(
                f"DEBUG: Середній час на обробку елемента: {avg_processing_time:.2f} сек",
                file=sys.stderr,
            )
            print(
                f"DEBUG: Останні {len(recent_times)} виміри часу: {[round(t, 2) for t in recent_times]}",
                file=sys.stderr,
            )
            ...
            if self.waiting_times:
                ...
                print(
                    f"DEBUG: Середній час очікування: {avg_waiting_time:.2f} сек",
                    file=sys.stderr,
                )
            ...
            safety_factor = 1.0
            if (
                len(self.elapsed_times) < 5
                or self.processed_items < self.total_items * 0.1
            ):
                if len(self.elapsed_times) == 1:
                    safety_factor = 1.2
                elif len(self.elapsed_times) < 3:
                ...
                print(
                    f"DEBUG: Застосовано коефіцієнт безпеки {safety_factor:.2f}x до часу обробки",
                    file=sys.stderr,
                )
            ...
            if remaining_processing is not None:
                print(
                    f"DEBUG: Прогноз часу обробки: {format_time(remaining_processing)} ({remaining_processing:.2f} сек)",
                    file=sys.stderr,
                )
                print(
                    f"DEBUG: Прогноз часу очікування: {format_time(remaining_waiting)} ({remaining_waiting:.2f} сек)",
                    file=sys.stderr,
                )
                print(
                    f"DEBUG: Загальний прогноз часу: {format_time(remaining_total)} ({remaining_total:.2f} сек)",
                    file=sys.stderr,
                )
        ...
        if remaining_total is not None:
            ...
            if debug_output and processing_time > 0 and waiting_time > 0:
                processing_percentage = (
                    processing_time / (processing_time + waiting_time)
                ) * 100
                total_processing_time = (
                    processing_time + remaining_processing
                    if remaining_processing is not None
                    else processing_time
                )
                total_waiting_time = (
                    waiting_time + remaining_waiting
...
def get_connection_string():
    ...
    auth_method = os.getenv(
        "OLAP_AUTH_METHOD", AUTH_SSPI
    ).upper()  # За замовчуванням SSPI
...
# Функція для підключення через OleDbConnection з System.Data.OleDb
def connect_using_oledb(connection_string, auth_details):
    """
    Підключається до OLAP сервера через OleDbConnection з System.Data.OleDb
    ...
    Returns:
        tuple: (connection, cursor) - OleDbConnection та курсор для запитів

    Notes:
        Використовує System.Data.OleDb з .NET для підключення.
    ...
    try:
        ...
        # Створюємо об'єкт OleDbConnection
        ...
        if os.getenv("OLAP_AUTH_METHOD", "").upper() == AUTH_LOGIN:
            ...
            if user_match and password_match:
                ...
                if save_credentials(username, password, encrypted=use_encryption):
                    print_success(
                        f"Облікові дані успішно збережено"
...
class OleDbCursor:
    """
    Клас-обгортка для OleDbConnection, щоб забезпечити спільний інтерфейс з pyadomd
    """
    ...
    def fetchall(self):
        ...
        fields = {}
        for i in range(self.reader.FieldCount):
            fields[i] = self.reader.GetName(i)
...
def handle_auth_error(
    e, auth_method, retry_count, connection_string, auth_details, is_oledb=False
):
    """
...
def connect_to_olap(connection_string=None, auth_details=None, retry_count=1):
    ...
    try:
        ...
        if auth_method == AUTH_LOGIN and (
            "User ID=" in connection_string and "Password=" in connection_string
        ):
            # Використовуємо OleDbConnection з System.Data.OleDb
            print_info(
                f"Використовуємо підключення через OleDbConnection для автентифікації за логіном/паролем"
            )
...
def run_mdx_query(connection, reporting_period):
    ...
    try:
        ...
        estimated_query_time = 120  # 2 хвилин у секундах
        ...
        try:
            ...
            if duplicate_columns:
                print_warning(
                    f"Деякі стовпці не були перейменовані через потенційне дублювання:"
                )
            ...
            def export_to_csv(file_path):
                ...
                )  # з BOM для коректного відображення кирилиці в Excel
...
def get_available_weeks(connection):
    ...
    try:
        ...
        for row in rows:
            ...
            if year_value is not None and week_value is not None:
                try:
                    ...
                    print_warning(
                        f"Помилка при конвертації значень {year_value}, {week_value}: {e}"
                    )
                    continue
        ...
        null_rows = sum(
            1
            for row in rows
            if convert_dotnet_to_python(row[0]) is None
            or convert_dotnet_to_python(row[1]) is None
        )
        if null_rows > 0:
...
try:
    ...
    query_timeout = int(
        os.getenv("QUERY_TIMEOUT", 30)
    )  # Значення за замовчуванням 30 секунд
